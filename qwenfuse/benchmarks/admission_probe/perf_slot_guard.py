...
def worker(a):
    ...
    summary = dict(variant=a.variant,mode=a.mode,kv_gib=a.kv_gib,samples=a.samples,threshold=a.threshold,
        ...
        note='Burst workload, explicit threshold in settings, synchronous scheduler, fixed C4 configuration; not sustained-server throughput. TTFT uses frontend latency; TPOT uses engine monotonic timestamps.')
...
def main():
    ...
    p.add_argument('--variant',choices=['BEFORE','AFTER'],required=True)
    ...
    before=Path(os.environ['QWENFUSE_SLOT_GUARD_BEFORE']).read_text()
    current=(REPO/'vllm/v1/core/sched/scheduler.py').read_text()
    old="""            if self.cache_aware_window and not defer_prefills:
                self._reorder_waiting_by_cached_prefix()
"""
    new="""            # Match the admission slot limit before probing cached prefixes.
...
"""
    ...
    assert before.replace(old,new)==current, 'Unexpected source differences: stop comparison'
    chosen=before if a.variant=='BEFORE' else current
    ast.parse(chosen)
    (overlay/'vllm/v1/core/sched/scheduler.py').write_text(chosen)
    (a.out/'scheduler-source.json').write_text(json.dumps(dict(
        variant=a.variant,sha256=hashlib.sha256(chosen.encode()).hexdigest()),indent=2))
    env=os.environ.copy()
    env.update(PYTHONPATH=os.pathsep.join([str(overlay),str(HERE)]),
